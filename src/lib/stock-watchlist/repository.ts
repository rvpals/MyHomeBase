...
export class SqliteStockWatchListRepository implements StockWatchListRepository {
  ...
  listWatchLists(): StockWatchList[] {
    const rows = this.db
      .prepare("SELECT * FROM stk_stock_watch_lists ORDER BY created_at ASC")
      .all() as WatchListRow[];
    return rows.map(watchListToDomain);
  }

  getWatchListById(id: number): StockWatchList | undefined {
    const row = this.db.prepare("SELECT * FROM stk_stock_watch_lists WHERE id = ?").get(id) as
      | WatchListRow
      | undefined;
  ...
  createWatchList(input: CreateWatchListInput): StockWatchList {
    const result = this.db.prepare("INSERT INTO stk_stock_watch_lists (name) VALUES (@name)").run(input);
  ...
  renameWatchList(id: number, input: RenameWatchListInput): StockWatchList {
    this.db.prepare("UPDATE stk_stock_watch_lists SET name = @name WHERE id = @id").run({ ...input, id });
  ...
  deleteWatchList(id: number): void {
    const deleteItems = this.db.prepare("DELETE FROM stk_stock_watch_list_items WHERE watch_list_id = ?");
    const deleteList = this.db.prepare("DELETE FROM stk_stock_watch_lists WHERE id = ?");
    this.db.transaction(() => {
      deleteItems.run(id);
      deleteList.run(id);
    })();
  ...
  listItems(watchListId: number): StockWatchListItem[] {
    const rows = this.db
      .prepare("SELECT * FROM stk_stock_watch_list_items WHERE watch_list_id = ? ORDER BY added_date ASC")
      .all(watchListId) as WatchListItemRow[];
    return rows.map(itemToDomain);
  }

  getItemById(id: number): StockWatchListItem | undefined {
    const row = this.db.prepare("SELECT * FROM stk_stock_watch_list_items WHERE id = ?").get(id) as
      | WatchListItemRow
      | undefined;
  ...
  addItem(input: AddWatchListItemInput, priceWhenAddedCents: number): StockWatchListItem {
    const result = this.db
      .prepare(
        `INSERT INTO stk_stock_watch_list_items (watch_list_id, ticker, shares, price_when_added_cents, added_date)
  ...
  updateItemReminder(id: number, input: UpdateWatchListItemReminderInput): StockWatchListItem {
    this.db
      .prepare(
        `UPDATE stk_stock_watch_list_items
  ...
  deleteItem(id: number): void {
    this.db.prepare("DELETE FROM stk_stock_watch_list_items WHERE id = ?").run(id);
  }
}
