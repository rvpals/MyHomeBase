...
const GROUPS: TableReferenceGroup[] = [
  {
    ...
    tables: [
      ...
      ["sys_fav_photo", "Starred photographs shown by the Picture Gallery module, keyed by path relative to the photo root."],
  ...
  },
  {
    ...
    tables: [
      ["inv_investment_accounts", "The brokerage accounts a position or transaction belongs to, with initial and latest value."],
      ["inv_stock_positions", "Current holdings, keyed by account and ticker — quantity, prices, value, cost basis and unrealised gain/loss."],
      ["inv_stock_transactions", "Individual buy, sell and dividend transactions, deduplicated so re-importing a broker CSV is a safe no-op."],
      ["inv_daily_snapshots", "One row per calendar day of whole-portfolio value and gain/loss, split by stock, ETF and other."],
      ["inv_account_performance_records", "The manual performance history of one account — its total value on a given date."],
      ["inv_tax_lots", "Historical purchase lots for the Tax Lot Analyzer, with a flag saying whether the shares are already split-adjusted."],
      ["inv_stock_watch_lists", "Named watch lists."],
      ["inv_stock_watch_list_items", "The tickers on a watch list, with hypothetical shares, the price when added and an optional reminder."],
      ["inv_ticker_favorites", "One row per starred ticker, read back as a newest-first jump list on the dashboard."],
      ["inv_ticker_logos", "Cached ticker logos. A row with no image is a deliberate negative cache — \"looked, none exists\"."],
      ["inv_ticker_monitors", "Per-ticker alert conditions on unrealized gain or loss, evaluated after a price refresh. `is_triggered` is the fire-once latch that stops one condition filing a message on every run."],
      ["inv_ticker_profiles", "Cached sector and industry per ticker, plus a manual sector override that wins over the fetched value."],
      ["inv_ticker_risk_cache", "Computed risk figures for one ticker's Risks card — volatility, 52-week range, market correlation, annualised return."],
      ["inv_stock_volatility_cache", "Portfolio-wide volatility, one row per held ticker. Rebuilt wholesale by the analytics recompute."],
      ["inv_stock_correlation_cache", "Single row caching the latest portfolio correlation matrix as JSON."],
      ["inv_stock_sharpe_cache", "Single row caching the last Sharpe-ratio run — its inputs and its full results."],
    ],
  },
  {
    prefix: "csv_",
  ...
  },
  {
    ...
    tables: [
      ...
      ["exp_rule_types", "The editable grouping for the rules list, keyed by the name a rule's type_name references. Display and filtering only — nothing in matching reads it."],
    ],
  },
  {
    prefix: "att_",
  ...
  },
  {
    ...
    tables: [
      ...
      ["mus_track_video", "Cached YouTube video pick per track, fetched only when \"Find video\" is pressed. A miss stays retryable — YouTube can't say a song has no video, only that we didn't find one."],
