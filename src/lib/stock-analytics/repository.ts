...
export class SqliteStockAnalyticsRepository implements StockAnalyticsRepository {
  ...
  listVolatilityCache(): VolatilityResult[] {
    const rows = this.db
      .prepare("SELECT * FROM inv_stock_volatility_cache ORDER BY type, annualized_vol_pct ASC")
  ...
  saveVolatilityCache(results: VolatilityResult[]): void {
    const upsert = this.db.prepare(
      `INSERT INTO inv_stock_volatility_cache
  ...
  clearVolatilityCache(): void {
    this.db.prepare("DELETE FROM inv_stock_volatility_cache").run();
  }

  getCorrelationCache(): CorrelationResult | undefined {
    const row = this.db.prepare("SELECT * FROM inv_stock_correlation_cache WHERE id = 1").get() as
      | CorrelationRow
      | undefined;
  ...
  saveCorrelationCache(result: CorrelationResult): void {
    this.db
      .prepare(
        `INSERT INTO inv_stock_correlation_cache
  ...
  clearCorrelationCache(): void {
    this.db.prepare("DELETE FROM inv_stock_correlation_cache").run();
  }

  getSharpeCache(): SharpeResult | undefined {
    const row = this.db.prepare("SELECT * FROM inv_stock_sharpe_cache WHERE id = 1").get() as
      | SharpeRow
      | undefined;
  ...
  saveSharpeCache(result: SharpeResult): void {
    this.db
      .prepare(
        `INSERT INTO inv_stock_sharpe_cache
