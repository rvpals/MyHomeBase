...
// Upload / replace / remove the home dashboard's background picture, and tune
...
//
// The picture uploads immediately rather than waiting for a Save button (the
// file is already chosen; parking megabytes in form state buys nothing), but the
// three knobs DO have a Save: they're a set you adjust together while watching
// the preview, and writing on every drag would be a request per pixel.

import { useRef, useState } from "react";
import { Button } from "@/components/button";
import { MAX_DASHBOARD_TEXTURE_BYTES, type DashboardTextureMode } from "@/lib/dashboard-texture";
import { IMAGE_UPLOAD_MIME_TYPES } from "@/lib/shared/image-upload";
import {
  removeDashboardTextureImageAction,
  saveDashboardTextureImageAction,
...
export function DashboardTextureControl({
  ...
  hasImage: boolean;
  /** The stored `updatedAt`, used as the preview's initial cache-buster. */
  imageVersion: string;
  opacity: number;
  ...
  // Bumped after an upload so the preview refetches — the route sends a
  // 5-minute max-age, so without this a replaced picture shows the old bytes.
  const [version, setVersion] = useState(imageVersion);

  const [opacity, setOpacity] = useState(initialOpacity);
  const [mode, setMode] = useState<DashboardTextureMode>(initialMode);
  const [blur, setBlur] = useState(initialBlur);

  const imageUrl = `/api/dashboard/texture?v=${encodeURIComponent(version)}`;
  ...
  async function handleFile(file: File | undefined) {
    ...
    try {
      ...
      const result = await saveDashboardTextureImageAction(body);
      if (result.ok) {
        setPresent(true);
  ...
  async function handleRemove() {
    ...
    setError(undefined);
    setSavedNote(undefined);
    const result = await removeDashboardTextureImageAction();
    if (result.ok) {
      setPresent(false);
  ...
  async function handleSaveSettings() {
    ...
    setError(undefined);
    setSavedNote(undefined);
    const result = await saveDashboardTextureSettingsAction({ opacity, mode, blur });
  ...
  return (
    <div className="mt-8 space-y-6">
      {/* The upload row. Same shape as the module carousel's control, so the two
          admin screens feel like one app. */}
      <div className="flex flex-wrap items-center gap-4 rounded-lg border border-line p-3">
        <span className="flex h-20 w-20 shrink-0 items-center justify-center overflow-hidden rounded-xl border border-line bg-paper">
          {present ? (
            ...
            <img
              src={imageUrl}
              alt="Dashboard background picture"
      ...
      {/* The knobs. Hidden with no picture: three controls that visibly do
          nothing are worse than an explanation of what to do first. */}
      {present && (
        <div className="rounded-lg border border-line p-4">
          ...
          <p className="mt-0.5 text-xs text-muted">
            ...
            deliberate — the dashboard&apos;s cards sit on top of this.
          </p>
